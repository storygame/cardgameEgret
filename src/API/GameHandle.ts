class GameHandle extends eui.Component{
			...
			if (this._condition(game_info, c, card_effect, condition_time)) {
				...
				for (let o in player["condition_list"]) {
					if (player["condition_list"][o] == c) {
						...
						// condition移除动画
						let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ConditionEvent, SingleBattleEvent.removeCondition, player, c);
						SingleBattlePage.instance().dispatchEvent(event);
					}
				}
				//执行该condition
				if (c["buff_turn"] > 0) {
			...
			if (this._condition(game_info, c, card_effect, condition_time)) {
				...
				for (let o in player["secret_condition_list"]) {
					if (player["secret_condition_list"][o] == c) {
						...
						// 显示移除一个secret动画
						let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.SecretEvent, SingleBattleEvent.removeSecret, player, c);
						SingleBattlePage.instance().dispatchEvent(event);
					}
				}
				//执行
			...
			if (game_info["buff_list1"][o]["buff_turn"] <= 0) {
				...
				for (let player_o in player["buff_list"]) {
					if (player["buff_list"][player_o] == tmp_c) {
						...
						// buff移除动画
						let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.BuffEvent, SingleBattleEvent.removeBuff, player, tmp_c);
						SingleBattlePage.instance().dispatchEvent(event);
					}
				}
			}
	...
	_damage(game_info, player, damage) {
		...
		if (damage > 0) {
			if (player["protect_special"].length > 0) {
				for (let o in player["protect_special"]) {
					if (player["protect_special"][o]['value'] == 999 && player["protect_special"][o]['times'] > 0) {
						...
						if(protect_once){
							let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ProtectEvent, SingleBattleEvent.immunityOnce, player, undefined);
							SingleBattlePage.instance().dispatchEvent(event);
						}else{
							let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ProtectEvent, SingleBattleEvent.immunityDamage, player, undefined);
							SingleBattlePage.instance().dispatchEvent(event);
						}
						return player;
					}
			...
			if (player["protect_value"] > 0) {
				if (player["protect_value"] > damage) {
					...
					damage = 0;
          			//显示护甲扣除动画
					let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ProtectEvent, SingleBattleEvent.decreaseProtect, player, undefined);
					SingleBattlePage.instance().dispatchEvent(event);
					...
					player["protect_value"] = 0;
          			// 护甲扣除动画，碎甲动画
					let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ProtectEvent, SingleBattleEvent.destoryProtect, player, undefined);
					SingleBattlePage.instance().dispatchEvent(event);
				}
			}
		}
		if (damage > 0) {
			...
			player["cute_value"] -= damage;
      		// 显示血量扣除动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.HPEvent, SingleBattleEvent.decreaseHP, player, undefined);
			SingleBattlePage.instance().dispatchEvent(event);
		}
		if (damage < 0) {
			//受到治愈,血量增加
			...
			player["cute_value"] -= damage;
      		// 显示血量增加动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.HPEvent, SingleBattleEvent.increaseHP, player, undefined);
			SingleBattlePage.instance().dispatchEvent(event);
		}
		//bound limit
	...
	_card(game_info, player, cards) {
		...
		if (cards == 999) {
			cards = player["cards_pool"].length;
     		// 显示抽出所有牌并爆牌，爆掉牌库 动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.CardEvent, SingleBattleEvent.cards999, player, player["cards_pool"]);
			SingleBattlePage.instance().dispatchEvent(event);
		}
		if (cards == -999) {
			cards = -1 * player["cards_value"];
      		// 显示弃掉所有手牌动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.CardEvent, SingleBattleEvent.cards_999, player, undefined);
			SingleBattlePage.instance().dispatchEvent(event);
		}
		...
		if (cards > 0) {
			...
			for (var i = 0; i < cards; i++) {
				if (player["cards_pool"].length > cardpool_lower) {
					...
					if (player["cards_value"] < cards_upper) {
						...
						let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.CardEvent, SingleBattleEvent.drawCardSuccess, player, new_card['effect']);
						SingleBattlePage.instance().dispatchEvent(event);
					}else{
            			// 显示抽牌并爆牌动画
						// console.log('[##]new_card: ', new_card)
						let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.CardEvent, SingleBattleEvent.drawCardFail, player, new_card['effect']);
						SingleBattlePage.instance().dispatchEvent(event);
          }
				} else {
					//无牌可抽,扣疲劳
					//如果疲劳是-999的话表示不计算疲劳
          			// 显示疲劳动画
					let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.CardEvent, SingleBattleEvent.drawNoCardTired, player, undefined);
					SingleBattlePage.instance().dispatchEvent(event);
					if (player["tired"] != -999) {
			...
			cards = cards * -1;
      		for(let dis_card in player["discard_list"]){
       		//显示弃一张牌动画
				let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.CardEvent, SingleBattleEvent.disCard, player, dis_card['effect']);
				SingleBattlePage.instance().dispatchEvent(event);
  ...
  // 基本动画执行函数 -- 费用
	_star(player, star) {
		...
		if (star == 999) {
			star = upper;
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ManaEvent, SingleBattleEvent.increaseToFullMana, player, undefined);
			SingleBattlePage.instance().dispatchEvent(event);
      // 显示费用增满动画 -- a/b 中 b 变化
		}
		if (star == -999) {
			star = -1 * upper;
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ManaEvent, SingleBattleEvent.decreaseToZeroMana, player, undefined);
			SingleBattlePage.instance().dispatchEvent(event);
      // 显示费用归零动画 -- a/b 中 b 变化 
		}
		if (star > 0) {
			...
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ManaEvent, SingleBattleEvent.increaseMana, player, undefined);
			SingleBattlePage.instance().dispatchEvent(event);
      // 费用增加动画
		} else {
			//减少总体令
			player["star_value"] += star;
			if(star != 0){
				let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ManaEvent, SingleBattleEvent.decreaseMana, player, undefined);
				SingleBattlePage.instance().dispatchEvent(event);
			}
      // 费用减少动画
		}
  ...
  // 基本动画执行函数 -- 护甲
	_protect(player, protect) {
		...
		if (protect > 0) {
			...
			player["protect_value"] += protect;
     		// 显示叠甲动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ProtectEvent, SingleBattleEvent.increaseProtect, player, undefined);
			SingleBattlePage.instance().dispatchEvent(event);
		} else {
			//护甲减少
			player["protect_value"] += protect;
	...
	_turn(game_info, player, turn) {
		...
		if (turn > 0) {
			game_info["extra_turn"][playerString] += turn;
      		// 显示获得一个额外回合动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ExtraTurnEvent, SingleBattleEvent.increaseExtraTurn, player, undefined);
			SingleBattlePage.instance().dispatchEvent(event);
		}
	...
	_addCondition(player, card_effect) {
		...
		if (player["condition_list"].length == GameRules.MaxConditionNum) {
      		//显示添加一个condition失败动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ConditionEvent, SingleBattleEvent.addConditionSuccess, player, card_effect);
			SingleBattlePage.instance().dispatchEvent(event);
			return false;
		} else {
			player["condition_list"].push(card_effect);
      		// 显示添加condition成功动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.ConditionEvent, SingleBattleEvent.addConditionFail, player, card_effect);
			SingleBattlePage.instance().dispatchEvent(event);
			return true;
		}
	}
	...
	_addSecret(player, card_effect) {
		...
		if (player["secret_condition_list"].length == GameRules.MaxSecretNum) {
      		//显示添加一个Secret失败动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.SecretEvent, SingleBattleEvent.addSecretFail, player, card_effect);
			SingleBattlePage.instance().dispatchEvent(event);
			return false;
		} else {
			player["secret_condition_list"].push(card_effect);
      		//显示添加一个Secret成功动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.SecretEvent, SingleBattleEvent.addSecretSuccess, player, card_effect);
			SingleBattlePage.instance().dispatchEvent(event);
			return true;
		}
	}
	...
	_addBuff(player, card_effect) {
		if (player["buff_list"].length == GameRules.MaxBuffNum) {
      		//显示添加一个buff失败动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.BuffEvent, SingleBattleEvent.addBuffFail, player, card_effect);
			SingleBattlePage.instance().dispatchEvent(event);
			return false;
		} else {
			...
			player["buff_list"].push(card_effect);
      		//显示添加一个buff成功动画
			let event = new SingleBattleEvent(SingleBattleEvent.SINGLEBATTLEEVENT, SingleBattleEvent.BuffEvent, SingleBattleEvent.addBuffSuccess, player, card_effect);
			SingleBattlePage.instance().dispatchEvent(event);
			return true;
		}
	}
	...
	IsFinished(game_info) {
		...
		if(!finished){
			...
			if(tie){
					SingleBattlePage.instance().TieGame();
				}else{
					if(winner['id'] == UserInfo.userid){
						SingleBattlePage.instance().EndGame(1);
					}else{
						SingleBattlePage.instance().EndGame(2)
					}
				}
		}
	}
